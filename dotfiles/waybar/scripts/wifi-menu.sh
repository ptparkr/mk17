...
# Prompt for password
get_password() {
  rofi -dmenu -password -config "${config}" -theme-str "${override_password}" -p " " || true
}
...
while true; do
  ...
  *"enabled"*)
    selected_option="$(echo "$options"$'\n'"$(wifi_list)" |
      rofi -dmenu -i -selected-row 1 -config "${config}" -p " " || true)"
    ;;
  *"disabled"*)
    selected_option="$(echo "$option_disabled" |
      rofi -dmenu -i -config "${config}" -theme-str "${override_disabled}" || true)"
    ;;
  esac
  ...
  *"Manual Entry")
    # Prompt for SSID
    manual_ssid="$(rofi -dmenu -config "${config}" -theme-str "${override_ssid}" -p " " || true)"
