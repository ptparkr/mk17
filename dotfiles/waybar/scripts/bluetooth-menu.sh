...
while true; do
  ...
  if [[ "$bluetooth_status" == "yes" ]]; then
    selected_option="$(echo -e "$options" | rofi -dmenu -i -selected-row 1 -config "${config}" -p " " || true)"
  else
    selected_option="$(echo -e "$option" | rofi -dmenu -i -selected-row 1 -config "${config}" -theme-str "${override_disabled}" -p " " || true)"
  fi
